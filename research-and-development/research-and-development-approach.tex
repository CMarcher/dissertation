...
\begin{tabularx}{\textwidth}{|X|>{\raggedright\arraybackslash}X|}
    ...
    Average (mean) request completion rate & The mean number of requests that were successfully completed per second. \\ \hline
    Maximum request completion rate & The peak request completion rate across the test duration. \\ \hline
    Maximum request start rate & The peak number of requests started by the k6 test client per second. \\ \hline
    Average (median) response time & The median time taken for a request to receive a response. \\ \hline
    Overall error rate & The proportion of requests across the test that failed due to an HTTP error or client timeout. \\ \hline
    Performance degradation threshold & The point at which the response time exceeds a certain level (100 milliseconds for UOR tests, and 1000 milliseconds for FS tests). This statistic allows for an approximation of the request start and completion rates where system performance begins to degrade rapidily. This threshold will not be calculated for average load or stress load tests.\\ \hline
    Maximum started and completed request ratio & The largest ratio between the number of requests that have started and the number that have completed. A high ratio indicates that the system is failing to process as many requests as it is receiving. This will not be calculated for breakpoint load tests.\\ \hline
    Maximum cluster CPU request proportion & The peak proportion of available cluster CPU used by a deployment. This will be calculated for the Django and IDAES pods only, and only for the resource allocation and utilisation tests. \\ \hline
    \caption{Summary statistics to be calculated}
    \label{table:analysis-summary-statistics}
\end{tabularx}
